        ...
        if line.startswith("diff --git a/"):
            ...
            if repo.language == 'python':
                ...
                if flag != "test" and not is_java:
                    flag = None
            elif repo.language in ['cpp', 'c++']:
                file_name = line.split("/")[-1]
                # Recognize C++ source and header files
                is_cpp = file_name.endswith(('.cpp', '.cc', '.cxx', '.hpp', '.h', '.hh', '.hxx'))
                # Recognize test files by path or name
                is_test = (
                    "test" in words or "tests" in words or "testing" in words or
                    file_name.startswith("test") or file_name.endswith("_test.cpp") or file_name.endswith("Test.cpp")
                )
                if flag != "test" and not is_cpp:
                    flag = None
                elif is_test:
                    flag = "test"
